...
function getCategoria() {
  $.ajax({
    url: "http://129.80.254.55:8080/api/Category/all",
    type: "GET",
    datatype: "JSON",
...
function putCategoria(idBotonActualizar) {
  ...
  if ($("#name").val().length == 0 || $("#description").val().length == 0) {
    alert("Todos los campos son obligatorios para actualizar la categoria");
  } else {
    let cajas = {
    ...
    $.ajax({
      url: "http://129.80.254.55:8080/api/Category/update",
      type: "PUT",
      datatype: "JSON",
  ...
  swalWithBootstrapButtons
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: "http://129.80.254.55:8080/api/Category/" + idBotonBorrar,
          type: "DELETE",
          datatype: "JSON",
          contentType: "application/JSON",
